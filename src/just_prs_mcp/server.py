...
def build_server(mode: Mode | None = None, settings: Settings | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "## Tool modes (essentials vs extended)\n"
            "The default 'essentials' surface covers the whole common workflow: "
            "catalog search/lookup, VCF normalization, single/batch/by-trait PRS, "
            "percentile / absolute-risk / quality analysis, the curated by-trait "
            "report, panel plotting, and public sample-genome download. **Switch to "
            "extended mode** (`PRS_MCP_MODE=extended` env var, or `--mode extended` "
            "on the CLI) only for these capabilities, which are intentionally gated "
            "out of the default surface:\n"
            "- explicit PGS-ID lists / arbitrary batch beyond a trait panel;\n"
            "- raw curation overrides on the by-trait report (preprints, "
            "non-ancestry-matched or toy scores, raw match-rate / method tuning);\n"
            "- reference-panel scoring and PLINK2 pgen scoring (needs pgenlib);\n"
            "- bulk catalog download / metadata export / HuggingFace upload;\n"
            "- prevalence-prior inspection and multi-method absolute-risk bundles.\n"
            "Essentials tools set `data.needs_extended=true` (with the switch name) "
            "in their result when a request would need one of the above, so you can "
            "tell the user to re-launch in extended mode rather than guessing from a "
            "missing tool.\n\n"
            "## Getting genotypes onto the server\n"
            "Computation tools (`compute_prs`, `compute_prs_batch`, "
            "`compute_prs_by_trait`, `normalize_vcf`) take **server-side** file "
            "paths — a path the client can see is not necessarily reachable here. "
            "For a remote/hosted server the only built-in ingest path is "
            "`download_sample_genome` (public Zenodo sample WGS). There is "
            "deliberately **no** tool to upload or fetch a user's own VCF to the "
            "server: persisting personal genomic/medical data server-side is a "
            "privacy/compliance liability we decline. A user with their own genome "
            "should run the server locally (stdio) against their own filesystem. "
            "Enumerate already-cached server paths via `list_genomes` / "
